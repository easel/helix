...
- **Test Plans**: `STP-002` defines `bash tests/helix-cli.sh` as the primary deterministic proof lane for wrapper and tracker behavior (`docs/helix/03-test/test-plans/STP-002-helix-cli.md:6-16`, `docs/helix/03-test/test-plans/STP-002-helix-cli.md:18-79`).
...
| `STP-002` still cites generic workflow sections as evidence for tracker metadata coverage rather than the authoritative contract text or deterministic harness locations that actually prove those behaviors. | stale | `docs/helix/03-test/test-plans/STP-002-helix-cli.md:96-106`; `workflows/README.md:164-166` | Low: the test plan remains usable, but evidence traceability is weak and stays a documentation gap. | hx-945d8bf2 |
...
| FEAT-002 | Running `bash tests/helix-cli.sh` remains the required deterministic verification path for wrapper behavior changes. | `bash tests/helix-cli.sh` | SATISFIED | `docs/helix/03-test/test-plans/STP-002-helix-cli.md:12-16`; this rerun executed `bash tests/helix-cli.sh` successfully. |
...
| Test-plan evidence traceability | STALE_PLAN | `docs/helix/03-test/test-plans/STP-002-helix-cli.md:18-94`; `workflows/README.md:164-166` | `docs/helix/03-test/test-plans/STP-002-helix-cli.md:96-106`; `bash tests/helix-cli.sh` passed in this rerun | plan-to-code | hx-4e5451cd |
...
| Test-plan evidence remains durable | Evidence references point at current supporting sources | `STP-002` | n/a | n/a | `docs/helix/03-test/test-plans/STP-002-helix-cli.md` | n/a | `STP-002` still cites generic workflow sections instead of direct contract or harness evidence | STALE_PLAN |
...
| hx-4e5451cd | task | `helix,phase:test,area:docs,source:review,feature:FEAT-002` | Refresh `STP-002` evidence references so they cite authoritative contract or deterministic harness sources | none | `docs/helix/03-test/test-plans/STP-002-helix-cli.md` cites current supporting evidence only |
...
5. `hx-4e5451cd` to repair the stale `STP-002` evidence references.
